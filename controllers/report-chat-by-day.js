...
exports.index = {
    json: function(req, res) {
        ...
        _async.waterfall([function(cb) {
            ...
            _async.parallel({
                offlineThread: function(next) {
                    _ChatThread.count({ isOffline: 1 }, next);
                },
                onlineThread: function(next) {
                    var agg = [{
                            $match: {
                                $and: [{
                                        $or: [
                ...
                res.json({ code: 200, data: { online: resp.onlineThread, offline: resp.offlineThread } });
            });
        });
        // _async.waterfall([function (cb) {
