...
var titleExport = 'BÁO CÁO THỐNG KÊ TỔNG QUAN HỘI THOẠI CHAT THEO AGENT';
var titleExportDetail = 'BÁO CÁO THỐNG KÊ CHI TIẾT HỘI THOẠI CHAT THEO AGENT';
const { headerReport } = require(path.join(_rootPath, 'commons', 'handleExcel', 'headerExcel.js'));
exports.index = {
    json: function (req, res) {
        if (!_.has(req.query, 'chatDetail')) {
                    ...
                    if (req.query.exportExcel) {
                        exportExcel(req, conditions, total, (err, link) => {
                            let paginator = new pagination.SearchPaginator({
                                prelink: '/report-statistic-chat-by-agent',
                                current: page,
                                rowsPerPage: rows,
                                totalResult: count
                            });
        ...
        if (_.has(req.query, 'chatDetail')) {
            let page = _.has(req.query, 'pageDetail') ? parseInt(req.query.pageDetail) : 1;
            let rows = _.has(req.query, 'rows') ? parseInt(req.query.rows) : 10;
            let aggDetail = [];
            aggDetail._pipeline = bindAggsDetail(req, res).aggs;
            let conditions = {};
            conditions['agg'] = bindAggsDetail(req, res).aggs;

            _async.parallel({
                sum: function (next) {
                    _ChatThread.aggregate(bindAggsDetail(req, res).aggs).exec(next)
                }
            }, function (error, rs) {
                if (error) {
                    return res.json({ code: 500 });
                }
                _ChatThread.aggregatePaginate(aggDetail, { page: page, limit: rows }, function (err, resp, pageCount, count) {
                    if (err) {
                        return res.json({ code: 500 });
                    }
                    let total = count;
                    if (req.query.exportExcel) {
                        exportExcelDetail(req, conditions, total, bindAggsDetail(req, res), (err, link) => {
                            let paginator = new pagination.SearchPaginator({
                                prelink: '/report-statistic-chat-by-agent',
                                current: page,
                                rowsPerPage: rows,
    ...
    html: function (req, res) {
        _async.parallel({
            ...
            _.render(req, res, 'report-statistic-chat-by-agent', {
                title: titleExport,
    ...
    _async.waterfall([
        ...
        function createExcelFilez(_config, callback) {
            ...
            _.each(excelHeader, function (header) {
                rowHeader.push(_config.MESSAGE.REPORT_STATISTIC_CHAT_BY_AGENT[header]);
            });
...
function exportExcelDetail(req, conditions, totalResult, { startDay, endDay }, cb) {
    let maxRecordPerFile = 65000;
    // let maxParallelTask = 1;
    let waterFallTask = [];
    let currentDate = new Date();
    let folderName = req.session.user._id + "-" + currentDate.getTime();
    let fileName = titleExportDetail + ' ' + _moment(currentDate).format('DD-MM-YYYY');
    console.log('folderName', folderName);
    // linkFile = path.join('assets', 'export', 'export-ternal', folderName + '.zip');
    let date = new Date().getTime();
...
function createExcelFileDetail(req, folderName, lastFolderName, fileName, data, { startDay, endDay }, callback) {
    let options = {
        filename: path.join(_rootPath, 'assets', 'export', 'cdr', folderName, fileName + '.xlsx'),
        useStyles: true,
        useSharedStrings: true,
    ...
    _async.waterfall([
        ...
        function createExcelFilez(_config, callback) {
            ...
            workbook.creator = req.session.user.displayName;
            workbook.created = new Date();
            let sheet = workbook.addWorksheet(titleExportDetail);
            ...
            headerReport(startDay, endDay, titleExportDetail, sheet, {});
            ...
            // sheet.getRow(2).font = { name: EXCEL_CONFIG.fontName, size: 13, bold: true };
            // sheet.getRow(3).font = { name: EXCEL_CONFIG.fontName, size: 10, bold: false };
            // sheet.getRow(6).font = { name: EXCEL_CONFIG.fontName, size: 18, bold: true };
            // sheet.getRow(9).font = { name: EXCEL_CONFIG.fontName, size: 11, bold: true };
            ...
            if (data !== null) {
                _async.eachSeries(data, function (agent, callback) {
                    _.each(agent.channel, function (el) {
                        sheet.addRow([
                            agent.agentId ? agent.agentId.displayName : '',
...
function setWeightColumn(worksheet) {
    let valueWidthColumn = [
        15, 25, 20, 20, 20, 20, 30, 30, 20, 30, 30, 30
    ]
...
function bindAggs(req, res) {
    ...
    let aggs = [
        ...
        {
            $lookup: {
                from: "companychannels", localField: "channelId", foreignField: "_id", as: "channelSLA"
            }
        },
        { $unwind: { path: '$channelSLA', preserveNullAndEmptyArrays: true } },
        {
            $project: {
                agentId: 1,
                // nameAgent: 1,
                ...
                createDate: 1,
                // eventDateGMT: 1,
                slaTimeReceive: { $multiply: ['$channelSLA.slaTimeReceive', 1000] },
                chatTime: {
                    $cond: [
                        {
                            $and: [
                ...
                waitTime: {
                    ...
                    //         ]
                    //     },
                    //     { $cond: [{ $ne: ["$agentAnswerMessageFirstTime", ""] }, { $add: [{ $subtract: ['$agentAnswerMessageFirstTime', '$createDate'] }, 25200000] }, 0] },
                    //     0
                    // ]
                    $cond: [
...

        {
            $group: {
                ...
                sumMsg: { $sum: { $size: '$messagesChatDetail' } },
                date: { $last: "$date" },
                createDate: { $last: "$createDate" },
                agentResponseCount: { $sum: "$agentResponseCount" },
                customerMessageCount: { $sum: "$customerMessageCount" },
                checkSLA: { $sum: { $cond: [{ $gt: ["$waitTime", "$slaTimeReceive"] }, 1, 0] } }
            }
        },
        {
            $lookup: {
                from: "users", localField: "_id", foreignField: "_id", as: "agentId"
            }
        },
        { $unwind: { path: '$agentId', preserveNullAndEmptyArrays: true } },
        {
...
function bindAggsDetail(req, res) {
    ...
    let aggs = [
        ...
        {
            $group: {
                _id: { channelId: "$channelId", agentId: "$agentId" },
                // nameAgent: { $last: "$nameAgent" },
        ...
        {
            $lookup: {
                from: "companychannels", localField: "channelId", foreignField: "_id", as: "channelId"
            }
        },
