...
var titleExport = 'BÁO CÁO THỐNG KÊ MAIL THEO THÁNG';
exports.index = {
        ...
        if (req.query.exportExcel) {
          exportExcel(req, conditions, total, (err, link) => {
            let paginator = new pagination.SearchPaginator({
              prelink: '/report-statistic-email-by-month',
              current: page,
              rowsPerPage: rows,
              totalResult: count
            });
  ...
  html: function (req, res) {
    _async.parallel({
      ...
      console.log(result);
      _.render(req, res, 'report-statistic-email-by-month', {
        title: titleExport,
...
function exportExcel(req, conditions, totalResult, cb) {
  ...
  waterFallTask.push(
    function (objectId, callback) {
      _async.parallel({
        archiver: function (callback) {
          fsx.mkdirs(path.join(_rootPath, 'assets', 'export', 'report-statistic-email-by-month'), callback);
        },
    ...
    function (t, callback) {
      ...
      let folderZip = path.join(_rootPath, 'assets', 'export', 'report-statistic-email-by-month', folderName + '.zip');
...
function createExcelFile(req, folderName, lastFolderName, fileName, data, { startDay, endDay }, callback) {
  ...
  _async.waterfall([
    ...
    function createExcelFilez(_config, callback) {
      ...
      headerReport(startDay, endDay, titleExport, sheet, { month: true });

      let rowHeader = []
      _.each(excelHeader, function (header) {
        rowHeader.push(_config.MESSAGE.REPORT_STATISTIC_EMAIL_BY_MONTH[header]);
      });
      ...
      if (data !== null) {
        ...
        data.forEach(function (el) {
          index += 1;
          el.data.forEach(function (inner_el, inner_index) {
            sheet.addRow([
              index,
              el._id,
              inner_el.nameSource,
            ...
            sheet.lastRow.font = { name: EXCEL_CONFIG.fontName, size: 11 };
            for (let i = 1; i <= excelHeader.length; i++) {
              let charNameColumn = _.columnToLetter(i);
              sheet.lastRow.getCell(charNameColumn).border = {
                top: { style: "thin" },
...
function bindAggs(req, res) {
  ...
  if (_.has(req.query, 'created') && req.query.created) {
    ...
    startDay = startDay.startOf('month')._d;
    endDay = endDay.endOf('month')._d;
    matchQuery.created = {
