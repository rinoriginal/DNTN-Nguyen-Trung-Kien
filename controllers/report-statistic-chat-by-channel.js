...
var titleExport = 'BÁO CÁO THỐNG KÊ HỘI THOẠI CHAT THEO KÊNH';
...
exports.index = {
    json: function (req, res) {
        ...
        let rows = _.has(req.query, 'rows') ? parseInt(req.query.rows) : 5;
        let agg = [];
                ...
                if (req.query.exportExcel) {
                    exportExcel(req, conditions, total, (err, link) => {
                        let paginator = new pagination.SearchPaginator({
                            prelink: '/report-statistic-chat-by-channel',
                            current: page,
                            rowsPerPage: rows,
                            totalResult: count
                        });
    ...
    html: function (req, res) {
        _async.parallel({
            ...
            _.render(req, res, 'report-statistic-chat-by-channel', {
                title: 'BÁO CÁO THỐNG KÊ HỘI THOẠI CHAT THEO KÊNH',
    ...
    _async.waterfall([
        ...
        function createExcelFilez(_config, callback) {
            ...
            _.each(excelHeader, function (header) {
                rowHeader.push(_config.MESSAGE.REPORT_STATISTIC_CHAT_BY_CHANNEL[header]);
            });
...
function bindAggs(req, res) {
    ...
    let aggs = [
        ...
        {
            $lookup: {
                from: "companychannels", localField: "_id", foreignField: "_id", as: "channel"
            }
        },
