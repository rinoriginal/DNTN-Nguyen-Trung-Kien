...
exports.create = function (req, res) {
    ...
    _Router.findOne({$and: [{link: req.body.link}, {link: {$ne: 'none'}}]}, function (error, menu) {
        ...
        if (_.isEqual(req.body.link, 'none') || _.getModule(req.body.link)) {
            ...
            req.body.parent = _.convertObjectId(req.body.parent)
            mongoClient.collection('routers').insertOne(req.body, function (error, _r) {
                res.json({code: (error ? 500 : 200), message: error ? (error.message ? error.message : error) : _r});
            });
        } else {
