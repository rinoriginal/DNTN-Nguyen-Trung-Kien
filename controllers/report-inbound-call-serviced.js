...
function getCdr(req, res, callback) {
    ...
    _async.waterfall([
        ...
        function (serviceId, callback) {
            ...
            if (_.has(req.query, 'agentId')) agg['agentId'] = {$in:_.arrayObjectId(req.query.agentId)};

            if (_.has(req.query, 'updated')) {
                let tempDate = req.query.updated.split(" - ");
                startDate =  _moment(tempDate[0], "DD/MM/YYYY");
                endDate = _moment(tempDate[1], "DD/MM/YYYY");
            ...
            getReportTCDCallTypeAgentDetail(startDate, endDate, page, rows, req.query.download, (err, result) => {
                if (err) return console.log(err, null);

                createPaging(req, startDate, endDate, page, rows);
                callback(err, result);
            });
        }
    ], callback);
...
function createPaging(req, startDate, endDate, page, rows) {
// function createPaging(req, agg, page, rows) {
    ...
    getTotalPaging(startDate, endDate, duration_GroupField, wait_GroupField, (err, total) => {
...
function exportExcel(req, res, conditions, totalResult) {
    ...
    if (totalResult > maxRecordPerFile) {
        ...
        var temp = function (callback) {
            ...
            getReportTCDCallTypeAgentDetail(conditions.startDate, conditions.endDate, null, null, req.query.download, (err, result) => {
...
function getReportTCDCallTypeAgentDetail(startDate, endDate, pages, rows, download, callback) {
    ...
    getRequestDefault(_config.cisco.apiCisco, `reportTCDCallTypeAgentDetail/detailAgent`, options, function(err, data) {
...
function getTotalPaging(startDate, endDate, duration_GroupField, wait_GroupField, callback) {
    ...
    let options = {
        ...
        // download,
        paging: "paging=1",
        duration_g: `duration_g=${duration_GroupField}`,
        wait_g: `wait_g=${wait_GroupField}`,
        callType: _config.cisco.apiCisco.callType,
        skillGroup: _config.cisco.apiCisco.skillGroup,
    };
...
function mappingData(data) {
    ...
    lastData = data.map(item => {
        let endTime = _moment(item.DateTime).add(item.TimeZone, "minutes").valueOf();
        item._id = item.PeripheralCallKey;
        item.company = _config.app.name;
        item.caller = item.ANI;
        item.called = item.Variable2;
        item.user = item.AgentPeripheralNumber;
        item.waitDuration = (item.Duration - item.TalkTime)*1000;
        item.startTime = _moment(item.FirstTimeCall).valueOf();
        item.callDuration = item.TalkTime*1000;
        item.answerTime = endTime - item.Duration*1000;
        item.endTime = endTime;

        if(item.agentInfo){
            item.user = item.agentInfo.displayName;
        }
        return item;
    });
