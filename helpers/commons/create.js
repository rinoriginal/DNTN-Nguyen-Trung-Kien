/**
 * Gán mặc định các menu khi tạo mới 1 nhà hàng
 * @param {String} idRestaurant id của nhà hàng
 */
...
function decodeMessageChat(content) {
    // decode content 
            ...
            if ((el.search("}bUid") != -1)
                ...
                if (el.search("}bPvtMsg") != -1) {
                    if (_moment(el.replace("S{", "").replace("}bPvtMsg", "").trim(), _moment.ISO_8601, true).isValid()) {
                        itemContent = {
                            ...itemContent,
                            createAt: new Date(el.replace("S{", "").replace("}bPvtMsg", "").trim())
                        }
                    }
                ...
                if (el.search("}mDeptId") != -1) {
                    if (_moment(el.replace("S{", "").replace("}mDeptId", "").trim(), _moment.ISO_8601, true).isValid()) {
                        itemContent = {
                            ...itemContent,
                            createAt: new Date(el.replace("S{", "").replace("}mDeptId", "").trim())
                        }
                    }
                ...
                if (el.search("}mMid") != -1) {
                    if (_moment(el.replace("S{", "").replace("}mMid", "").trim(), _moment.ISO_8601, true).isValid()) {
                        itemContent = {
                            ...itemContent,
                            createAt: new Date(el.replace("S{", "").replace("}mMid", "").trim())
                        }
                    }
                ...
                if (el.search("}}") != -1) {
                    if (_moment(el.replace("S{", "").replace("}}", "").trim(), _moment.ISO_8601, true).isValid()) {
                        itemContent = {
                            ...itemContent,
                            createAt: new Date(el.replace("S{", "").replace("}}", "").trim())
                        }
                    }
                }
