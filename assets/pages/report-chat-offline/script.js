...
var DFT = function ($) {
  ...
  $('.zmdi-refresh').bind('click', function () {
    _.LoadPage(location.hash = "report-chat-offline");
  });
  ...
  var queryFilter = function (isDownload, page) {
    ...
    _Ajax("/report-chat-offline?" + $.param(filter), 'GET', {}, function (resp) {
  ...
  var loadHistory = function (resp) {
    ...
    let templateBoxChat = `<div role="tabpanel" class="tab-pane active" id="tab-list-chat">` +
      `<div class="panel-body msg_container_base">` +
      showMessage(resp.data[0]) +
      `</div>` +
      `</div>`
    let templateInfo = ` <div role="tabpanel" class="tab-pane " id="tab-info-chat">` +
      ...
      `</div>` +
      `<div class="col-md-12 p-0"` +
      `style="max-height: 413px;overflow-y: scroll;overflow-x: hidden;">` +
      `<table class=" table table-borderless col-md-12 p-0 m-b-5 box-shadow">` +
      resp.str +
      // `<tr class=" p-0">`+
      //     `<td class="col-md-4 m-8 left-side ">1</td>`+
      //     `<td class="col-md-8 m-8 right-side">2</td>`+
      // `</tr>`+
      // `<tr class=" p-0">`+
      //     `<td class=" col-md-4 m-8 left-side">3</td>`+
      //     `<td class="col-md-8 m-8 right-side">4</td>`+
      // `</tr>`+
      `</table>` +
      `</div>` +
      `</div>` +
    ...
    let templateNote = `<div role="tabpanel" class="tab-pane " id="tab-detail-chat">` +
      `<div class="panel-body p-0">` +
      `<div class="title">` +
  ...
  var showMessage = function (data) {
    ...
    data.messagesChat.forEach(function (item) {
      if (item.type == 'agent') {
        listMessage.push(`<div class="row msg_container base_sent" style="padding:0">` +
          `<div class="col-md-10 col-xs-10">` +
          `<div class="text-right"><span datetime="2009-11-13T20:00">${item.name} • ${moment(item.createAt).format('HH:mm:ss DD/MM/YYYY')}</span></div>` +
      ...
      if (item.type == 'customer') {
        listMessage.push(`<div class="row msg_container base_receive" style="padding:0">` +
          `<div class="col-md-10 col-xs-10">` +
          `<div class="text-left"><span datetime="2009-11-13T20:00">${item.name} • ${moment(item.createAt).format('HH:mm:ss DD/MM/YYYY')}</span></div>` +
  ...
  var showInfoConver = function (data) {
    if (!data) return ``;
    return `<div class="col-md-8 p-0 right-side" style="width:262px">` +
  ...
  var loadData = function (resp) {
    ...
    var template = '<tr>' +
      ...
      '<a role="button" data-id="{6}" data-form="complaint" href="javascript:void(0)" class="p-t-3 btn-flat-bg btn-show" data-toggle="tooltip" data-placement="top" data-original-title="Thông tin hội thoại">' +
      '<i style="color:#3F8498" class="zmdi zmdi-comments  f-17"></i></a> </td>' +
      '</tr>';
