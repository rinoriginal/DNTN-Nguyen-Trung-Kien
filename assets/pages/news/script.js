var DFT = function ($) {
    ...
    var bindClick = function () {
        ...
        $(document).on('click', '#btnCreateNew', handleEventClickBtnCreateNew);

        function handleEventClickBtnCreateNew(e) {
            e.preventDefault();
            let data = {
                title: $("#title").val(),
            ...
            let params = {};
            params.idCategory = getUrlParams(window.location.href).idCategory;
            ...
            for (var i = 0, len = filesToUpload.length; i < len; i++) {
                filesToUpload[i].file.name = filesToUpload[i].file.name.replace( /[ ]/g,"");
                formData.append("files", filesToUpload[i].file);
            }

            $.ajax({
                url: "/news?" + $.param(params),
                data: formData,
                processData: false,
                contentType: false,
                type: "POST",
    ...
    function renderFileToContent(files) {
        ...
        $("#tblListFile tbody").html(
            Object.keys(files).map((i, index) => {
                ...
                return "<tr>" + [
                    `<td>Tên tài liệu</td>`,
                    `<td class="text-center">${moment(i.lastModifiedDate).format('HH:mm DD/MM/YYYY')}</td>`,
                    `<td>${files[i].name}</td>`,
    ...
    return {
        init: function () {
            ...
            // $('.page-loader').show();
            renderStatusBrand("create_new", $("#create_new_status").is(":checked"));
        ...
        uncut: function () {
            ...
            $(document).off('click', '#btnCreateNew');
