var DFT = function ($) {
  ...
  var getFilterChat = function (firstLoad, exportExcel, page) {
    ...
    const apiURL = '/report-statistic-email-by-day?' + $.param(filter);
    ...
    _Ajax(apiURL, 'GET', {}, function (resp) {
      ...
      loadSum(resp);
      loadChart(resp);
      $('#pagingReportEmail').append(_.paging('#report-statistic-email-by-day', resp.paging));
      return;
    })
  };
  ...
  */
  var bindValueEmailByDay = function () {
    _.each(_.allKeys(_config.MESSAGE.REPORT_STATISTIC_EMAIL_BY_DAY), function (item) {
      $('.' + item).html(_config.MESSAGE.REPORT_STATISTIC_EMAIL_BY_DAY[item]);
    });
  };
