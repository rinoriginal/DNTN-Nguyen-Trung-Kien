...
var DFT = function ($) {
    ...
    var getFilter = function () {
        ...
        _Ajax("/report-outbound-agent?"+ $.param(filter),'GET', {}, function(resp){
            console.log({resp});
            if(resp.code==200){
                if(resp.data.length){
                    datas= resp.data;
    ...
    var bindClick = function () {
        ...
        $('#btnExcel').on('click', function (event) {
            ...
            var btnExcel = tableToExcel('exceldata', 'My Worksheet');
            $(this).attr('download', todaysDate + '_Báo cáo gọi ra - Năng suất ĐTV - Theo thời gian.xls')
            $(this).attr('href', btnExcel);
        })
    ...
    var cascadeOption = function () {
        $('select[name="idCompany"]').on('change', function () {
            $.get('/report-outbound-agent', {cascade: $(this).val()}, function (res) {
