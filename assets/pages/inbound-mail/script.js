...
var DFT = function ($) {
    ...
    var bindSocket = function (client) {
        ...
        client.on('responseTicketInboundMailPagingData', function (resp) {
            ...
            if (_.has(pagingObject, resp.formId) && index >= 0) {
                ...
                if (resp.code == 200) {
                    console.log('32 ----------- ', createPaging(resp.message));
    ...
    var bindClick = function () {
        ...
        $(document).on('click', '.click-to-call', function (e) {
            _socket.emit('MakeCallReq', { _id: user, sid: _socket.id, ticket: makeCallData[$(this).attr('data-ticket')], number: $(this).attr('data-number') });
        });
    };
    ...
    var requestTickets = function (formId, dateTime, url, ignoreSearch) {
        ...
        _AjaxData(url, 'GET', null, function (resp) {
            console.log(258, resp)
            if (resp.code == 500 || (resp.message.length == 0)) {
                swal({
    ...
    var loadData = function (formId, resp) {
        ...
        resp.message.forEach(function (el) {
            ...
            var inlineEditSelectPicker =
                ...
                '</select>';
            console.log(369, el)
            if (!_.isNull(el.deadline) && el.status !== 2) {
            ...
            rows += template.str(el.company,
                ...
                !!el.field_e_mail ? el.field_e_mail : '',
                status,
    ...
    return {
        ...
        uncut: function () {
            ...
            lastPagingData = {};
            makeCallData = {};
            delete _socket.off('responseTicketInboundMailPagingData');
