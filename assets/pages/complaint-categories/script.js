...
var DFT = function ($) {
    ...
    var bindClick = function () {
        ...
        $('.complaint').on('click', function () {
            ...
            _AjaxData('complaint-categories' + '/' + _id, 'GET', {}, function (resp) {
        ...
        $('#modal-form-input-complaint').on('hidden.bs.modal', function (e) {
            setValueField($(this), [
                ...
                { fieldId: 'edit-nameComplaint', value: '', type: 'input' },
                { fieldId: 'edit-slaComplaint', value: '', type: 'input' },
                { fieldId: 'complaintId', value: '', type: 'input' },
        ...
        $('#modal-form-input-problem').on('hidden.bs.modal', function (e) {
            setValueField($(this), [
                ...
                { fieldId: 'edit-nameProblem', value: '', type: 'input' },
                { fieldId: 'edit-slaProblem', value: '', type: 'input' },
                { fieldId: 'form-modal-problem-name', value: '', type: 'input' },
                { fieldId: 'problemId', value: '', type: 'input' },
            ]);
        });
    ...
    var filterProblem = function (id) {
        ...
        _AjaxData('complaint-categories?' + $.param(filter), 'GET', {}, function (resp) {
    ...
    var bindSubmit = function () {
        ...
        $('#form-modal-complaint-custom').validationEngine('attach', {
            ...
            onValidationComplete: function (form, status) {
                ...
                if (type == 'add') {
                    var url = 'complaint-categories';
                    var method = 'POST';
                } else if (type == 'edit') {
                    var id = form.find('#complaintId').val();
                    var url = 'complaint-categories/' + id;
                    var method = 'PUT';
        ...
        $('#form-modal-problem-custom').validationEngine('attach', {
            ...
            onValidationComplete: function (form, status) {
                ...
                if (type == 'add') {
                    var url = 'problem-categories';
                    var method = 'POST';
                } else if (type == 'edit') {
                    var id = form.find('#problemId').val();
                    var url = 'problem-categories/' + id;
                    var method = 'PUT';
                }
                    ...
                    _AjaxData(url, method, formData, function (resp) {
                        if (_.isEqual(resp.code, 200)) {
                            ...
                            swal({
                                ...
                                function () {
                                    // _.LoadPage(window.location.hash);
                                    filterProblem(resp.data.idComplaint)
                                });
                        } else {
                            swal({ title: 'Thông báo!', text: resp.message, type: "error" });
                        }
    ...
    function renderProblem(data) {
        ...
        data.forEach(function (item) {
            row += `<tr>`;
            row += `<td class="text-center"> <span title="` + (item.nameProblem ? item.nameProblem : '') + ` ">` + (item.nameProblem ? item.nameProblem : '') + `</span> </td>` +
                `<td class="text-center"> <span>` + (item.status == 1 ? '<i class="fa fa-check text-success"></i>' : '') + `</span> </td>` +
                `<td class="text-center"> <span>` + (item.slaProblem ? item.slaProblem : '') + `</span> </td>` +
    ...
    function getDataEdit(_form, _id) {
        ...
        if (_form == 'complaint') {
            _AjaxData('complaint-categories/' + _id + '/edit', 'GET', $('#form-modal-custom').getData(), function (resp) {
                console.log({ resp });
                ...
                if (_.isEqual(resp.code, 200)) {
                    setValueField($('#modal-form-input-complaint'), [
                        ...
                        { fieldId: 'edit-nameComplaint', value: resp.data.nameComplaint, type: 'input' },
                        { fieldId: 'edit-slaComplaint', value: resp.data.slaComplaint, type: 'input' },
                        { fieldId: 'complaintId', value: resp.data._id, type: 'input' },
        ...
        if (_form == 'problem') {
            _AjaxData('problem-categories/' + _id + '/edit', 'GET', $('#form-modal-custom').getData(), function (resp) {
                if (_.isEqual(resp.code, 200)) {
                    setValueField($('#modal-form-input-problem'), [
                        ...
                        { fieldId: 'edit-nameProblem', value: resp.data.nameProblem, type: 'input' },
                        { fieldId: 'edit-slaProblem', value: resp.data.slaProblem, type: 'input' },
                        { fieldId: 'edit-idComplaint', value: data, selected: resp.data.idComplaint, type: 'select' },
                        { fieldId: 'form-modal-problem-name', value: _form, type: 'input' },
                        { fieldId: 'problemId', value: resp.data._id, type: 'input' },
                    ]);
            ...
            switch (item.type) {
                ...
                case 'select':
                    ...
                    _.each(item.value, function (o) {
                        var option = new Option(o.nameComplaint, o._id, false);
