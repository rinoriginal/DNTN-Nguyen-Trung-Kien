var DFT = function ($) {
    ...
    var bindClick = function () {
        ...
        $(document).on('change', '#idCompany', function(e){
            ...
            $('.option-s').remove();
            var url = '/skills-mail?type=groupsprofile&idCompany=' + $(this).find(":checked").val();
            $.get(url, function(res){
                _.each(res, function(g, i){
                    g.skillmail.index = i;
                    $('#group-skill').append(newOption_s(g.skillmail)).selectpicker('refresh');
                });
            });
        });
    };
            ...
            onValidationComplete: function (form, status) {
                if (status) {
                    ...
                    if (skills.length == 0){
                        ...
                        _Ajax('/groups-profile-mail', 'POST', [{data: JSON.stringify(profile)}], function (resp) {
    ...
    return {
        init: function () {
            ...
            $.get(urll, function(res){
                ...
                $('#group-skill').empty().selectpicker('refresh');
                $('.option-s').remove();
                var url = '/skills-mail?type=groupsprofile&idCompany=' + res.companies[0]._id;
                $.get(url, function(resp){
                    _.each(resp, function(g, i){
                        g.skillmail.index = i;
                        $('#group-skill').append(newOption_s(g.skillmail)).selectpicker('refresh');
                    });
                });
