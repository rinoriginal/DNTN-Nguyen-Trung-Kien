var DFT = function ($) {
    ...
    var bindClick = function () {
        ...
        $(document).on('click', '.btn-remove', function () {
            ...
            swal({
                ...
                function () {
                    _AjaxObject('/groups-profile/' + _id, 'DELETE', {}, function (resp) {
                        if(resp.code == 200){
                            swal({title: 'Thành công', text: _config.MESSAGE.GROUP_PROFILE.DELETE_TEXT_SUCCESS, type: "success"});
                            ...
                            swal({title: 'Thất bại', text: 'Xóa thất bại ' + failProfile, type: "warning"});
                            _.LoadPage(window.location.hash);
                        }
                    });
                });
        ...
        $(document).on('click', '#btn-delSelection', function(){
            ...
            swal({
                ...
                function () {
                    _Ajax('/groups-profile/all', 'DELETE', [{ids: _.compact(ids)}], function (resp) {
                        if(resp.code == 200){
                            swal({title: 'Thành công', text: _config.MESSAGE.GROUP_PROFILE.DELETE_TEXT_SUCCESS, type: "success"});
                            _.LoadPage(window.location.hash);
                        }else {
        ...
        $('#group-profile').on('keyup', function(e) {
    ...
    var bindSubmit = function () {
        $('#group-profile').submit(function(e){
            e.preventDefault();
        })
    };
