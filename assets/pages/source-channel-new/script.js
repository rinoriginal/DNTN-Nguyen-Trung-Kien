    ...
    var bindSubmit = function () {
        $(document).on('submit', '#add-new-channel', function(e){
            ...
            async.parallel({
                one: function(cb){
                    //validate tên kênh
                    $.get('/source-channel/validate?companyChannel=' + $('#idCompanyChannel').val() + '&name=' + $('#name').val(), function(resp){
                        if (!resp.code){
                ...
                if (resp.one){
                    ...
                    //Gửi request update
                    _AjaxObject('/source-channel', 'POST', $('#add-new-channel').getData(), function (resp) {
