...
var DFT = function ($) {
    ...
    var deleteOrg = function () {
        ...
        swal({
                title: _config.MESSAGE.MAIL_TEMPLATE.DELETE_TITLE,
                text: _config.MESSAGE.MAIL_TEMPLATE.DELETE_TEXT,
                type: "warning",
            ...
            function () {
                _Ajax('/mail-template/all', 'DELETE', [{ids: _.compact(ids)}], function (resp) {
                    if (_.isEqual(resp.code, 200)) {
                        swal({
                                title: _config.MESSAGE.MAIL_TEMPLATE.MSG_SUCCESS_TITLE,
                                text: _config.MESSAGE.MAIL_TEMPLATE.MSG_DELETE,
                                type: "success"
                            },
                            function () {
                        ...
                        swal({title: _config.MESSAGE.MAIL_TEMPLATE.MSG_FAIL_TITLE, text: resp.message});
                    }
                });
            });
    };
    ...
    var bindClick = function () {
        ...
        $('#search_company').on('change', function(){
            $.get('/mail-template-categories?status=1&companyId=' + $(this).val(), function(res){
                ...
                $('#search_category').append('<option selected value="">Tất cả</option>');
                _.each(res.data, function(g, i){
                    $('#search_category').append(newOption(g));
                });
        ...
        $(document).on('click', '.btn-remove', function () {
            ...
            swal({
                ...
                _AjaxObject('/mail-template/' + _id, 'DELETE', {}, function (resp) {
                    if (_.isEqual(resp.code, 200)) {
                        swal({
                                title: _config.MESSAGE.MAIL_TEMPLATE.MSG_SUCCESS_TITLE,
                                text: _config.MESSAGE.MAIL_TEMPLATE.MSG_DELETE,
    ...
    var bindTextValue = function () {
        ...
        $('#txt_template_status').html(_config.MESSAGE.MAIL_TEMPLATE.LIST_STATUS_COL_LABEL);
        $('#txt_template_actions').html(_config.MESSAGE.MAIL_TEMPLATE.LIST_ACTION_COL_LABEL);
    ...
    return {
        init: function () {
            if (isNullTemplates && Object.keys(window.location.obj).length > 0) {
                swal({
                    title: _config.MESSAGE.MAIL_TEMPLATE.SEARCH_NOT_FOUND_TITLE,
                    text: _config.MESSAGE.MAIL_TEMPLATE.SEARCH_NOT_FOUND_TEXT,
                    type: "warning",
                    confirmButtonColor: "#DD6B55",
