...
var DFT = function ($) {
    ...
    var deleteOrg = function () {
        ...
        swal({
                title: _config.MESSAGE.MAIL_SETTING.DELETE_TITLE,
                text: _config.MESSAGE.MAIL_SETTING.DELETE_TEXT,
                type: "warning",
            ...
            function () {
                _Ajax('/mail-setting/all', 'DELETE', [{ids: _.compact(ids)}], function (resp) {
                    if (_.isEqual(resp.code, 200)) {
                        swal({
                                title: _config.MESSAGE.MAIL_SETTING.MSG_SUCCESS_TITLE,
                                text: _config.MESSAGE.MAIL_SETTING.MSG_DELETE,
                                type: "success"
                            },
                            function () {
                        ...
                        swal({title: _config.MESSAGE.MAIL_SETTING.MSG_FAIL_TITLE, text: resp.message});
                    }
                });
            });
    };
    ...
    var bindClick = function () {
        ...
        $(document).on('click', '.btn-remove', function () {
            ...
            swal({
                ...
                _AjaxObject('/mail-setting/' + _id, 'DELETE', {}, function (resp) {
                    if (_.isEqual(resp.code, 200)) {
                        swal({
                                title: _config.MESSAGE.MAIL_SETTING.MSG_SUCCESS_TITLE,
                                text: _config.MESSAGE.MAIL_SETTING.MSG_DELETE,
    ...
    var bindTextValue = function () {
        $('#txt_service_name').html(_config.MESSAGE.MAIL_SETTING.LIST_NAME_COL_LABEL);
        $('#txt_service_status').html(_config.MESSAGE.MAIL_SETTING.LIST_STATUS_COL_LABEL);
        $('#txt_service_actions').html(_config.MESSAGE.MAIL_SETTING.LIST_ACTION_COL_LABEL);
    };

    return {
        init: function () {
            if (isNullService && Object.keys(window.location.obj).length > 0) {
                swal({
                    title: _config.MESSAGE.MAIL_SETTING.SEARCH_NOT_FOUND_TITLE,
                    text: _config.MESSAGE.MAIL_SETTING.SEARCH_NOT_FOUND_TEXT,
                    type: "warning",
                    confirmButtonColor: "#DD6B55",
