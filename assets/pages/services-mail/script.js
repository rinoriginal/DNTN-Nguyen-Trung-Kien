    ...
    var bindClick = function () {
        ...
        $(document).on('click', '.btn-remove', function () {
            ...
            swal({
                ...
                function () {
                    _AjaxObject('/services-mail/' + _id, 'DELETE', {}, function (resp) {
                        if(resp.code==500){
                            swal({title: 'Có lỗi xảy ra', text: resp.message, type: "error"} , function(){
                                //_.LoadPage(window.location.hash);
                            });
                        }else{
        ...
        $(document).on('click', '#btn-delSelection', function(){
            ...
            swal({
                ...
                function () {
                    _Ajax('/services-mail/all', 'DELETE', [{ids: _.compact(ids)}], function (resp) {
                        swal({title: 'Thành công', text: _config.MESSAGE.SERVICES.DELETE_TEXT_SUCCESS, type: "success"}, function(){
                            _.LoadPage(window.location.hash);
                        });
                    });
    ...
    var bindPressKey = function(){
        ...
        $('#search-bar').keypress(function (event) {
            ...
            if (key == 13) {
                ...
                if (searchValue !== ''){
                    window.location.hash = 'services-mail/search?keyword=' + searchValue;
                }
            }
        });
    }
