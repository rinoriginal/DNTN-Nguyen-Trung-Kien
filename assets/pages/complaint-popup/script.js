var DFT = function ($) {
    ...
    var optionRestaurant = function (data) {
        $('#restaurantId').empty()
        $('#restaurantId').append('<option value="" >---- Chọn ----</option>')
        data.forEach(function (item) {
            $('#restaurantId').append(newOption(item, ''));
        })
        $('#restaurantId').selectpicker('refresh');

    }
    var optionProblem = function (data) {
        $('#problemId').append('<option value="" >---- Chọn ----</option>')
        data.forEach(function (item) {
            $('#problemId').append('<option value="' + item._id + '" data-sla2="' + item.slaProblem + '">' + item.nameProblem + '</option>');
        })
        $('#problemId').selectpicker('refresh');

    }
    var optionSubCategory = function (data) {
        $('#subCategoryComplaintId').empty()
        $('#subCategoryComplaintId').append('<option value="" >---- Chọn ----</option>')
        data.forEach(function (item) {
            $('#subCategoryComplaintId').append(newOptionSubCategory(item, ''));
        })
        $('#subCategoryComplaintId').selectpicker('refresh');

    }
    var optionCategory = function (data) {
    ...
    var bindChange = function () {
        ...
        $('#typeComplaintId').on('change', function () {
            ...
            if (slaComplaint !== '') {
                $('#appointmentDate').val(moment().add(slaComplaint, 'h').format('DD/MM/YYYY HH:mm'));
            }
            if (complaintId != '') {
        ...
        $('#problemId').on('change', function () {
            ...
            $('#appointmentDate').val('')
            if (slaProblem != '') {
                $('#appointmentDate').val(moment().add(slaProblem, 'h').format('DD/MM/YYYY HH:mm'));
            }
        })
        //chọn category
    ...
    var bindSubmit = function () {
        $('#form-new-complaint').validationEngine('attach', {
            ...
            onValidationComplete: function (form, status) {
                ...
                if (status) {
                    ...
                    var check = false;
                    if (customerComplaint && customerComplaint != undefined) {
                        formData.append('customerId', customerComplaint._id);
                        check = true;
                    } else if ($('#btnSave').attr('data-id') != '') {
                        formData.append('customerId', $('#btnSave').attr('data-id'))
                        check = true;
                    } else {
                        swal({
                            title: 'Lỗi!',
                    ...
                    if (check) {
                        _AjaxData('complaint', 'POST', formData, function (resp) {
                            if (_.isEqual(resp.code, 200)) {
                                swal({
                                    title: 'Thông báo',
                                    text: resp.message,
                                    ...
                                    function () {
                                        if (customerComplaint) {
                                            window.parent.closeModalComplaint();
                                            // _Ajax("/manage-ticket-complaint", 'GET', {}, function (resp) {
    ...
    function loadData(formId, resp) {
        ...
        resp.message.forEach(function (el) {
            ...
            var buttonAction = `<audio id="${el._id}" class="myAudio" preload="none">` +
                ...
                '<input type="checkbox" class="scales" name="id" src="' + el.recordPath + '" data-record="' + el.callId + '">' +
                '</button>';
    ...
    return {
        ...
        uncut: function () {
            ...
            $('#form-new-complaint').validationEngine('detach');
        }
    };
}(jQuery);