var DFT = function ($) {
            ...
            var tags = _.Tags([
                {
                    tag: 'tr', childs: [
                    {
                        ...
                        content: moment(data.created).format('DD/MM/YYYY hh:mm')
                    },//thang
    ...
    var getFilter = function () {
        ...
        _Ajax("/report-chat-offline-by-month?"+ $.param(filter),'GET',{}, function(resp){
            if(resp.code==200){
                if (resp.data.length) {
                    initTable(resp.data);
    ...
    var initChart = function (datas){
        ...
        _.each(datas, function(data){
            categories.push(data._id.month + "/" + data._id.year);
        ...
        Highcharts.chart('container', {
            ...
            title: {
                text: 'Biểu đồ số lượng theo tháng'
            },
        ...
        _.each(datas, function(data, i){
            ...
            var tags= _.Tags([
                {tag:'tr', attr: {class: 'trBody', threadId: data.threadId}, childs: [
                    ...
                    {tag: 'td', attr: {class: 'text-center'}, content: (data.receive ? Math.round((data.answer / data.receive) * 100) : "0") + "%"},//ti le phuc vu
