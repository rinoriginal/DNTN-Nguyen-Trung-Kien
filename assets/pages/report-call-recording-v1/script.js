    ...
    function loadData(formId, resp) {
        ...
        resp.message.forEach(function (el) {
            ...
            var buttonAction = `<audio id="${el._id}" class="myAudio" preload="none">` +
                `<source  src="${ipRecording}/${el.recordingPath}" type="audio/wav">` +
                ...
                `<a href="${ipRecording}/` + el.recordingPath + '?download=true" download><i class="zmdi zmdi-download zmdi-hc-fw download-audio f-25" data-url="' + el.callId + '"></i></a>' +
                '</button>';
            ...
            rows += template.str(el.direction,
                ...
                el.extension,
                el.phone ? el.phone : '',
                moment(el.connectTime).format("DD/MM/YYYY"),
                moment(el.connectTime).format("HH:mm:ss DD/MM/YYYY"),
                moment(el.disconnectTime).format("HH:mm:ss DD/MM/YYYY"),
