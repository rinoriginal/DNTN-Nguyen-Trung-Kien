...
var DFT = function ($) {
    ...
    var bindClick = function () {
        ...
        $(document).on('click', '.click-to-call', function (e) {
            ...
            let companyId = dataTicket.idCampain.idCompany._id
            let campainId = dataTicket.idCampain._id
            sessionStorage.setItem("ticketId", currentTicketId);
            sessionStorage.setItem("companyId", companyId);
            sessionStorage.setItem("campainId", campainId);
            sessionStorage.setItem("customerNumber", customerNumber);
            // _socket.emit("clickToCallOutbound", { ticketId: currentTicketId, companyId: companyId, campainId: campainId, customerNumber: customerNumber});
            window._finesse.makeCall(customerNumber, getCookie('_extension'), function (_1, _2, xhr) {
                if (xhr && xhr.status === 202) {
                    intervalCisco = setInterval(function() {
                        getDialogsFromCisco(currentTicketId, companyId, campainId, customerNumber);
                    }, 1000);
                }
            }, _makeCallHandler);
        });
